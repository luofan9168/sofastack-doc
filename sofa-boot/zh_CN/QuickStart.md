...
`status: "UP"` 表示应用 Readiness Check 健康的。可以通过在浏览器中输入 `http://localhost:8080/health` 来查看应用的运行时健康状态（可能会随着时间发生变化）。SOFABOOT 3.X 中更改为 `http://localhost:8080/actuator`查看应用运行时健康状态。
